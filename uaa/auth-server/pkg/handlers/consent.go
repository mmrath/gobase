...
import (
	...
	"github.com/mmrath/gobase/uaa-server/pkg/helpers"
	"github.com/mmrath/gobase/uaa-server/pkg/hydra"
)
...
func ConsentGetHandler(hydraClient *client.OryHydra, templateProvider TemplateProvider) http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if getConsentResponse.GetPayload().Skip {
			...
			for i := 0; i < len(hydra.AutoScopes); i++ {
				if helpers.SliceIndex(len(acceptConsentRequestParams.Body.GrantedScope), func(j int) bool {
					return acceptConsentRequestParams.Body.GrantedScope[j] == hydra.AutoScopes[i]
				}) == -1 {
					acceptConsentRequestParams.Body.GrantedScope = append(acceptConsentRequestParams.Body.GrantedScope, hydra.AutoScopes[i])
				}
			}
